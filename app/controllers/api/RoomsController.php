...
//        }
    ...
    function activitiesAction()
    {
        ...
        if ($game_history) {
            ...
            // 房间红包
            if (isDevelopmentEnv()) {
                $red_packet_cond = ['conditions' => 'current_room_id=:current_room_id: and status != :status:',
                    'bind' => ['current_room_id' => $room_id, 'status' => STATUS_OFF]
                ];
                $red_packet_num = \RedPackets::count($red_packet_cond);
                $res['red_packet'] = ['num' => $red_packet_num, 'url' => 'url://m/games'];
            }
