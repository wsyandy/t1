...
class UsersController extends BaseController
{
    ...
    function registerAction()
    {
        ...
        if (!$data) {
            return $this->renderJSON(ERROR_CODE_FAIL, '请求参数错误');
        }

        $product_channel = $this->currentProductChannel();
        if (!$product_channel) {
            return $this->renderJSON(ERROR_CODE_FAIL, "产品渠道为空");
        }
        ...
        info("注册的用户信息", $user);
        if (!$user) {
            return $this->renderJSON(ERROR_CODE_FAIL, '注册失败');
        }

        return $this->renderJSON(ERROR_CODE_SUCCESS, 'OK', ['sid' => $user->sid]);
    }
