 ...
 * Time: 下午13:00
 */
class TurntableDrawHistories extends BaseModel
{
    /**
     * @type Users
     */
    ...
    function afterCreate()
    {
        ...
        // 系统总收入
        $cache_key = 'turntable_draw_history_total_amount_incr_' . $this->pay_type;
        $user_db->incrby($cache_key, intval($this->pay_amount));

        // 系统支出: 金币，钻石，礼物
        $cache_decr_key = 'turntable_draw_history_total_amount_decr_' . $this->type;
        $user_db->incrby($cache_decr_key, intval($this->number));

        $new_cache_key = 'new_v2_turntable_draw_history_total_amount_incr_' . $this->pay_type;
        $new_cache_decr_key = 'new_v2_turntable_draw_history_total_amount_decr_' . $this->type;
        ...
        if ($this->type == 'diamond' && $this->number == 100000
            ...
            $cache_hit_10w_key = 'turntable_draw_history_hit_all_notice';
    ...
    static function isDayLimit($data)
    {
        ...
        $user_db = Users::getUserDb();
        $cache_key = 'turntable_draw_history_hit_num_' . date('Ymd') . '_' . $id;
                    ...
                    if (!$user->isCompanyUser()) {
                        ...
                        foreach ($user_hit_10w_histories as $history) {
                            ...
                            if ($hit_user && ($hit_user->device_id == $user->device_id || $hit_user->ip == $user->ip)) {
                                ...
                                $user_db = Users::getUserDb();
                                $user_db->zadd('turntable_draw_histories_block_user_ids', time(), $user->id);
                                return 0;
                            }
    ...
    static function isBlockUser($user)
    {
        ...
        $user_db = Users::getUserDb();
        $score = $user_db->zscore('turntable_draw_histories_block_user_ids', $user->id);
    ...
    static function calculatePrize($user, $hit_diamond = false)
    {
        ...
        $user_db = Users::getUserDb();
        // 系统总收入
        $cache_key = 'turntable_draw_history_total_amount_incr_diamond';
        $total_incr_diamond = $user_db->get($cache_key);
        // 系统支出
        $cache_decr_key = 'turntable_draw_history_total_amount_decr_diamond';
        $total_decr_diamond = $user_db->get($cache_decr_key);

        $cache_gift_decr_key = 'turntable_draw_history_total_amount_decr_gift';
        ...
        foreach ($data as $datum) {
            ...
            if (fetch($datum, 'rate') * 10 * $user_rate_multi >= $random) {
                ...
                $user_db = Users::getUserDb();
                $cache_key = 'turntable_draw_history_hit_num_' . date('Ymd') . '_' . fetch($datum, 'id', 0);
                $user_db->incr($cache_key);
                $cache_total_key = 'turntable_draw_history_hit_num_' . fetch($datum, 'id', 0);
    ...
    static function createHistory($user, $opts = [])
    {
        ...
        $gift = null;
        $turntable_draw_history = new TurntableDrawHistories();
        $turntable_draw_history->user_id = $user->id;
        $turntable_draw_history->product_channel_id = $user->product_channel_id;
        $turntable_draw_history->type = fetch($result, 'type');
        $turntable_draw_history->number = fetch($result, 'number');
        $turntable_draw_history->pay_type = fetch($opts, 'pay_type');
        $turntable_draw_history->pay_amount = fetch($opts, 'pay_amount');

        $turntable_draw_history->gift_id = fetch($result, 'gift_id', 0);
        $turntable_draw_history->gift_num = fetch($result, 'gift_num', 0);
        if ($turntable_draw_history->gift_id) {
            $gift_id = fetch($result, 'gift_id', 0);
            $gift = Gifts::findFirstById($gift_id);
            $turntable_draw_history->gift_type = $gift->type;
        }

        if (!$turntable_draw_history->save()) {
            return null;
        }

        if ($turntable_draw_history->type == 'diamond') {
            $remark = '抽奖获得' . $turntable_draw_history->number . '钻石';
            $opts['remark'] = $remark;
            $target = \AccountHistories::changeBalance($user, ACCOUNT_TYPE_DRAW_INCOME, $turntable_draw_history->number, $opts);
        } elseif ($turntable_draw_history->type == 'gift') {
            if ($gift) {// 送礼物
                // 赠送权时间
            ...
            $opts = ['remark' => '抽奖获得' . $turntable_draw_history->number . '金币'];
            $target = \GoldHistories::changeBalance($user, GOLD_TYPE_DRAW_INCOME, $turntable_draw_history->number, $opts);
        }


        $hot_cache = TurntableDrawHistories::getHotWriteCache();
        if ($turntable_draw_history->number >= 1000 && $turntable_draw_history->number < 10000) {
            $hot_cache->zadd('turntable_draw_histories_1000', time(), $turntable_draw_history->id);
        }
        if ($turntable_draw_history->number >= 10000 && $turntable_draw_history->number < 100000) {
            $hot_cache->zadd('turntable_draw_histories_10000', time(), $turntable_draw_history->id);
        }
        if ($turntable_draw_history->number >= 100000) {
            $hot_cache->zadd('turntable_draw_histories_100000', time(), $turntable_draw_history->id);
        }

        return $turntable_draw_history;
    }
    ...
    {
        $user_db = Users::getUserDb();
        $relations_key = 'turntable_draw_histories_block_user_ids';
    ...
    static function deleteBlockUser($user_id)
    {
        if ($user_id) {
            $user_db = Users::getUserDb();
            $user_db->zrem("turntable_draw_histories_block_user_ids", $user_id);
        }
    }
