...
{% macro info(soft_version) %}
    ...
    版本号: {{ soft_version.version_code }}  版本名称: {{ soft_version.version_name }}
    {{ soft_version.status_text }} {{ soft_version.stable_text }}<br/>
    安装包内置fr: {{ soft_version.built_in_fr }}
{% endmacro %}

<a href="/admin/soft_versions/new" class="modal_action">添加新版本</a>
<form name="search_form" action="/admin/soft_versions" method="get" autocomplete="off">
    <label for="id_eq">ID</label>
    <input name="soft_version[id_eq]" type="text" id="id_eq"/>
    <label for="soft_version_status_eq">是否可用</label>
    <select name="soft_version[status_eq]" id="status_eq">
        {{ options(SoftVersions.STATUS) }}
    </select>
    ...
    <label for="soft_version_stable_eq">版本状态</label>
    <select name="soft_version[stable_eq]" id="stable_eq">
        {{ options(SoftVersions.STABLE) }}
    </select>
...
'更新简介': 'feature', '下载次数': 'download_num', '升级次数': 'updated_num', '更新时间':'updated_at_text','是否强制升级': 'force_update_text',
'备注': 'remark',
'下载地址': 'down_url', '操作者':'operator_username', '操作': 'edit_link']) }}
...
<script type="text/template" id="soft_version_tpl">
    <tr id="soft_version_${soft_version.id}">
        ...
        <td>${soft_version.force_update_text}</td>
        ...
        <td>
            <a href="/admin/soft_versions/edit/${soft_version.id}" class="modal_action">编辑</a>
        </td>
    </tr>
</script>