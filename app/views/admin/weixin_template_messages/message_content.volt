{%- macro add_message_content(push_message) %}
    ...
    {% else %}
        <a href="/admin/weixin_template_messages/add_message_content?id={{ push_message.id }}" id="add_message_content">添加</a>
    {% endif %}
{%- endmacro %}

<input type="hidden" id="weixin_template_message_id" value="{{ weixin_template_message_id }}">

{%- macro image_link(push_message) %}
    <img src="{{ push_message.image_small_url }}" height="50" width="50"/>
{%- endmacro %}

<a href="/admin/weixin_template_messages">返回</a>
...
<script type="text/javascript">
    $('body').on('click', '#add_message_content', function (e) {
        ...
        var weixin_template_message_id = $("#weixin_template_message_id").val();
        href += '&weixin_template_message_id=' + weixin_template_message_id;
