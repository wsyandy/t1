...
{%- macro product_channel_link(activity) %}
    <a class="modal_action"
       href="/admin/activities/product_channel_ids?id={{ activity.id }}">查看({{ activity.product_channel_num }})</a>
{%- endmacro %}
...
{{ simple_table(activities, ['id': 'id','标题': 'title','图片': 'image_link','状态':'status_text', '类型':'type_text','排序':'rank','code':'code',
...
<script type="text/template" id="activity_tpl">
    <tr id="activity_${activity.id}">
        ...
        <td><a class="modal_action" href="/admin/activities/platforms?id=${activity.id}">查看(${ activity.platform_num
                })</a></td>
        <td><a class="modal_action" href="/admin/activities/product_channel_ids?id=${activity.id}">查看(${
                activity.product_channel_num })</a></td>
