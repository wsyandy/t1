...
{{ f.select('material_type', ['label': '产品类型', 'collection': Banners.MATERIAL_TYPE, 'width':'33%']) }}
{{ f.input('material_ids', ['label': '房间ID', 'width':'33%']) }}

{{ f.select('status', ['label': '状态', 'collection': Banners.STATUS, 'width':'33%']) }}
